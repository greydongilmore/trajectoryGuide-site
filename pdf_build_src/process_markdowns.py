...
def extract_header_string():
    ...
    run_shell_cmd("cp ../mkdocs.yml docs_copy/")

    with open(os.path.join(os.path.dirname(__file__), 'docs_copy/mkdocs.yml'), 'r') as file:
        data = file.readlines()

    header_string = data[0].split(": ")[1]

    title = header_string.split()[0]
...
def remove_template_links(root_path):
    ...
    for root, dirs, files in os.walk(root_path):
        for file in files:
            if file.endswith(".md"):
                ...
                data = [ele for ele in data if not any(substring in ele for substring in {'---\n', 'template:', '<br>', '!!! note','#### Interactive'})]

                for ind, line in enumerate(data):
                    if 'title: ' in line:
                        data[ind] = '# '+line.split(':')[-1].strip()

                    elif '<img' in line and '.svg' not in line:
                        src=line.split('src=')[1].split(' ')[0].replace('"','').replace('../','')
                        alt=line.split('alt=')[1].split(' ')[0].replace('"','').replace('/>','')
                        data[ind] ='![{}]({})'.format(alt, src)

                    elif '<center>' in line:
                        data[ind] = line.replace('<center>','').replace('</center>','')
...
#
#                #for ind, line in enumerate(data):
#                #    if '<img' in line and '.svg' not in line:
#                #        src=line.split('src=')[1].split(' ')[0].replace('"','').replace('../','')
#                #        alt=line.split('alt=')[1].split(' ')[0].replace('"','').replace('/>','')
#                #        data[ind] ='![{}]({})'.format(alt, src)
#
...
if __name__ == '__main__':
    ...
    duplicated_src_dir_path = 'docs_copy/docs'
    ...
    remove_template_links(duplicated_src_dir_path)
